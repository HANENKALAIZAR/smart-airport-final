...
export default function FlightDetailPage() {
    ...
    const riskFactors = [
        { label: 'Weather Conditions', pct: Math.min(95, 48 + delay) },
        { label: 'Air Traffic Congestion', pct: Math.min(92, 35 + delay * 0.8) },
        { label: 'Aircraft Turnaround Time', pct: Math.min(88, 22 + delay * 0.6) },
        { label: 'Historical Performance', pct: Math.min(80, 30 + delay * 0.5) },
    ].sort((a, b) => b.pct - a.pct);

    // Use SHAP bars if available, else fallback
    const barData = prediction?.shap_explanation
        ? Object.entries(prediction.shap_explanation)
            .sort(([, a], [, b]) => Math.abs(b) - Math.abs(a))
            .slice(0, 4)
            .map(([label, val], idx) => ({
                label,
                pct: Math.round(Math.min(95, (Math.abs(val) / Math.max(...Object.values(prediction.shap_explanation).map(v => Math.abs(v)))) * 95)),
                explanation: riskExplanations[label] || '',
                isDominant: idx === 0,
            }))
        : riskFactors.map((f, idx) => ({ ...f, explanation: riskExplanations[f.label] || '', isDominant: idx === 0 }));
            ...
            <div className="pfdp__card">
                ...
                <div className="pfdp__row2">
                    ...
                    <div className="pfdp__info-card">
                        ...
                        <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', lineHeight: 1.5, marginBottom: 12 }}>
                            Our AI model has identified the following factors contributing to the delay risk:
                        </p>
