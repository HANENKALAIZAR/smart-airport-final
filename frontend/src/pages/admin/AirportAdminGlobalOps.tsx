...
// MAIN COMPONENT
// ════════════════════════════════════════════════════════════════════════════
export default function AirportAdminGlobalOps() {
    const { t } = useLanguage();
    ...
    const myAirport  = airports[0] ?? null;

    const radarMetrics: RadarMetric[] = myAirport ? [
        { label: 'OTP',          value: myAirport.on_time_rate ?? 0 },
        { label: 'Reliability',  value: Math.round((myAirport.reliability_score ?? 0) * 100) },
        { label: 'Low Risk',     value: myAirport.risk_level === 'Low' ? 100 : myAirport.risk_level === 'Medium' ? 50 : 0 },
        { label: 'Active Data',  value: myAirport.has_data ? 100 : 0 },
        { label: 'AI Predicted', value: myAirport.upcoming_predicted_flights > 0 ? 80 : 0 },
    ] : [];

    const totalFlights = myAirport?.total_historical_flights ?? 0;
    const airportOTP    = myAirport?.on_time_rate ?? 0;
    const avgDelay     = myAirport?.avg_delay_min ?? 0;
    const aiPredicted  = myAirport?.upcoming_predicted_flights ?? 0;
    ...
    return (
        <div style={{ color: 'var(--adm-text)' }}>
            ...
            <section style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                {/* Station Performance overview card */}
            ...
            {/* Analytics Lens (Heatmap + weather + operators) */}
            <AnalyticsTier />
        </div>
    );
}
