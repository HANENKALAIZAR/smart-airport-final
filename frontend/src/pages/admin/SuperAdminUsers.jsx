...
function RoleBadge({ role }) {
    if (role === 'super_admin') return <span className="users-badge users-badge--super">⭐ Super Admin</span>;
    return <span className="users-badge users-badge--admin">🛡 Admin</span>;
}
...
function IdDocBadge({ status, profileComplete }) {
    if (!profileComplete) {
        return <span style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.35)' }}>—</span>;
    }
    if (status === 'approved') return <span style={{ fontSize: '0.78rem' }}>🟢 Approved</span>;
    if (status === 'rejected') return <span style={{ fontSize: '0.78rem' }}>🔴 Rejected</span>;
...
export default function SuperAdminUsers() {
    ...
    const filtered = useMemo(() => users.filter(u => {
        ...
        return matchSearch && matchAirport && matchStatus;
    }), [users, search, airportFilter, statusFilter]);
    ...
    return (
        <div className="admin-page">
            ...
            <div className="users-stats">
                ...
                <div className="users-stat-card users-stat-card--gold"><Shield size={22} className="users-stat-icon" /><div><div className="users-stat-value">1</div><div className="users-stat-label">Super Admin</div></div></div>
            </div>
                        ...
                        <tbody>
                            ...
                            {!loading && filtered.map(user => (
                                <tr key={user.id}>
                                    ...
                                    <td>
                                        <IdDocBadge status={user.idDocumentStatus} profileComplete={user.profileComplete} />
                                    </td>
                            ...
                            {reviewDetail && !reviewLoading && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, fontSize: '0.82rem' }}>
                                        <div><span style={{ color: 'rgba(255,255,255,0.4)' }}>Phone</span><br />{reviewDetail.phone_number || '—'}</div>
                                        <div><span style={{ color: 'rgba(255,255,255,0.4)' }}>DOB</span><br />{reviewDetail.date_of_birth ? String(reviewDetail.date_of_birth).slice(0, 10) : '—'}</div>
                                        <div><span style={{ color: 'rgba(255,255,255,0.4)' }}>ID type</span><br />{reviewDetail.id_type || '—'}</div>
                                        <div><span style={{ color: 'rgba(255,255,255,0.4)' }}>ID number</span><br /><span style={{ fontFamily: 'monospace' }}>{reviewDetail.id_number || '—'}</span></div>
                                        <div style={{ gridColumn: '1 / -1' }}><span style={{ color: 'rgba(255,255,255,0.4)' }}>Personal email</span><br />{reviewDetail.personal_email || '—'}</div>
                                        <div style={{ gridColumn: '1 / -1' }}><span style={{ color: 'rgba(255,255,255,0.4)' }}>ID status</span><br /><IdDocBadge status={reviewDetail.id_document_status} profileComplete={!!reviewDetail.profile_complete} /></div>
                                    </div>
                                    ...
                                    <div>
                                        <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>ID document</div>
                                        {reviewDetail.id_document_url ? (
                                            reviewDetail.id_document_url.startsWith('data:application/pdf') ? (
                                                <iframe title="id" src={reviewDetail.id_document_url} style={{ width: '100%', height: 360, border: 'none', borderRadius: 8, background: '#fff' }} />
                                            ) : (
                                                <img src={reviewDetail.id_document_url} alt="ID" style={{ maxWidth: '100%', borderRadius: 8 }} />
                                            )
                                        ) : (
                                            <span style={{ color: 'rgba(255,255,255,0.35)' }}>No document on file.</span>
                                        )}
                                    </div>
